...
class TensorTermPortProtocolError(Error):
  pass
...
class TensorrDuplicateTermError(Error):
  pass
...
class TensorIndentationError(Error):
  pass


class TensorNextIpError(Error):
  pass


class TensorMultipleTerminatingActionError(Error):
  pass


class TensorFragmentInV6Error(Error):
  pass
...
class Config(object):
  ...
  def Append(self, line, verbatim=False):
    ...
    if line.endswith('}'):
      ...
      if self.indent < self._initial_indent:
        raise TensorIndentationError('Too many close braces.')
    spaces = ' ' * self.indent
...
class Tensor(aclgenerator.ACLGenerator):
    ...
    for header, terms in pol.filters:
      ...
      for term in terms:
        ...
        if term.name in term_names:
          raise TensorDuplicateTermError('You have multiple terms named: %s' %
                                          term.name)
        ...
        if 'is-fragment' in term.option and filter_type == 'inet6':
          raise TensorFragmentInV6Error('The term %s uses "is-fragment" but '
                                         'is a v6 policy.' % term.name)

        new_terms.append(self._TERM(term, filter_type, enable_dsmo, noverbose))

      self.tensor_policies.append((header, filter_name, filter_type,
